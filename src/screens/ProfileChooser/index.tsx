		...
		return (
			<WrapperView>
				<SafeAreaComponent>
					...
					<Title style={{ fontFamily: 'Inter_400Regular' }}>
						{' '}
						Who's watching?{' '}
					</Title>
					...
					<MainView
						...
						renderItem={({ item, index }: any) =>
							isNotTheLastElement(index) ? (
								<ProfileIcon
									onPress={() => handleProfileSelection(item.name, item.photo)}
									userName={item.name}
									size={100}
									source={item.photo}
								/>
								...
								<View style={{ alignItems: 'center' }}>
									<PressableAnimation
										onPress={() => Alert.alert(messageUnavailable)}
										animationState={animationGrow}>
										<MaterialIcons
									...
									<Text style={{ color: themes.COLORS.WHITE }}>
										Adicionar perfil
									</Text>
								</View>
							)
